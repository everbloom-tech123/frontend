...
class HomepageCategoryService {
  ...
  /**
   * NEW METHOD: Fetches active homepage categories with their experiences in a single call
   * This optimized method reduces the number of API calls needed
   * 
   * @param {number} limit - Optional limit for number of experiences per category
   * @returns {Promise<Array>} Array of active homepage categories with their experiences
   */
  static async getActiveHomepageCategoriesWithExperiences(limit = 5) {
    try {
      console.log('Fetching active homepage categories with experiences from:', 
        `${this.API_BASE_URL}/active-with-experiences${limit ? `?limit=${limit}` : ''}`);
      
      const response = await axios.get(`${this.API_BASE_URL}/active-with-experiences`, {
        params: { limit }
      });
      
      console.log('Active homepage categories with experiences fetched successfully:', response.data);
      return response.data;
    } catch (error) {
      console.error('Error fetching active homepage categories with experiences:', error);
      console.error('Error details:', error.response?.data);
      // Return empty array instead of throwing to prevent UI breaks
      return [];
    }
