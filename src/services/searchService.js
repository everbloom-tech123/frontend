...
const SearchBar = ({ searchOpen, setSearchOpen }) => {
    ...
    const searchProducts = async () => {
      if (query.trim()) {
        ...
        try {
          ...
          const response = await axios.get(`${config.API_BASE_URL}/public/api/products/search?query=${query}`);
  ...
  // Update image URL to use config
  const getImageUrl = (imageUrl) => {
    return `${config.API_BASE_URL}/public/api/products/files/${imageUrl}`;
  };
